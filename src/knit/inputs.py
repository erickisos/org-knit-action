...
from .colour import bgrey, bred, green, magenta, red, yellow
...
def _string_or(value, value_func):
    ...
    return value if isinstance(value, str) else value_func()
...
class Inputs:
    arg_def = {
        ...
        'branch': v_replace(
            bool_or_str, True, default_branch(work_dir)
        ),  # True
    ...
    def __init__(self, *args) -> None:
        if len(args) != len(self.arg_def):
            raise ValueError(
                f'Number of arguments passed ({len(args)}) does not match'
